...
It's also possible to download the toolbox matching your OS directly from the [[https://github.com/DavidRConnell/matlab-igraph/releases][release page]], however, this will need to be manually updated. If manually downloading the toolbox, double click on the file (ending in ~.mltbx~) in MATLAB to install.
