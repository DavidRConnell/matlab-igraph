...
#define VERIFY_N_INPUTS_EQUAL(expected)                                        \
  do {                                                                         \
    if (nrhs != (expected)) {                                                  \
      IGRAPH_FATALF("%s expected %d inputs got %d.", mexFunctionName(),        \
                    (expected), nrhs);                                         \
    }                                                                          \
  } while (0)
...
#define VERIFY_NO_INPUTS                                                       \
  if (nrhs != 0) {                                                             \
    IGRAPH_FATALF("%s expected no inputs got %d.", mexFunctionName(), nrhs);   \
  }                                                                            \
  (void)prhs
...
#define VERIFY_N_INPUTS_ATLEAST(bound)                                         \
  do {                                                                         \
    if (nrhs < (bound)) {                                                      \
      IGRAPH_FATALF("%s expected atleast %d inputs got %d.",                   \
...
#define VERIFY_N_INPUTS_ATMOST(bound)                                          \
  do {                                                                         \
    if (nrhs > (bound)) {                                                      \
      IGRAPH_FATALF("%s expected atmost %d inputs got %d.", mexFunctionName(), \
                    (bound), nrhs);                                            \
    }                                                                          \
  } while (0)
...
#define VERIFY_N_INPUTS_BETWEEN(lower, upper)                                  \
  do {                                                                         \
    if ((nrhs < (lower)) || (nrhs > (upper))) {                                \
      IGRAPH_FATALF("%s expected between %d and %d inputs got %d.",            \
...
#define VERIFY_N_OUTPUTS_EQUAL(expected)                                       \
  do {                                                                         \
    if (nlhs != (expected)) {                                                  \
      IGRAPH_FATALF("%s expected %d outputs got %d.", mexFunctionName(),       \
                    (expected), nlhs);                                         \
    }                                                                          \
  } while (0)
...
#define VERIFY_NO_OUTPUTS                                                      \
  if (nlhs != 0) {                                                             \
    IGRAPH_FATALF("%s expected no outputs got %d.", mexFunctionName(), nlhs);  \
  }                                                                            \
  (void)plhs
...
#define VERIFY_N_OUTPUTS_ATLEAST(bound)                                        \
  do {                                                                         \
    if (nlhs < (bound)) {                                                      \
      IGRAPH_FATALF("%s expected atleast %d outputs got %d.",                  \
...
#define VERIFY_N_OUTPUTS_ATMOST(bound)                                         \
  do {                                                                         \
    if (nlhs > (bound)) {                                                      \
      IGRAPH_FATALF("%s expected atmost %d outputs got %d.",                   \
                    mexFunctionName(), (bound), nlhs);                         \
    }                                                                          \
  } while (0)
...
#define VERIFY_N_OUTPUTS_BETWEEN(lower, upper)                                 \
  do {                                                                         \
    if ((nlhs < (lower)) || (nrhs > (upper))) {                                \
      IGRAPH_FATALF("%s expected between %d and %d outputs got %d.",           \
