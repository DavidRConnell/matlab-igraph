...
%   degree (in and out) of each node. Requires mode to be 'all'. If false, when
%   rewiring, edges are taken from one node pair and move to another at random.
%
%   GRAPH = REWIRE(GRAPH, 'mode', MODE) Whether to rewire both ends ('all',
%   default) only 'in' node or only 'out' node. Must be 'all' if preserving
%   degrees.
%
...
%   output name-value pairs. The output defaults to the input's values. If the
%   input is a sparse logical the output will be a sparse logical. Available
%   parameters are 'repr', 'dtype', and 'isdirected'. See the "Common IGRAPH
%   Arguments" section from help IGRAPH for more information.
%
